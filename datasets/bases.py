...
class ImageTextDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.args.dualstream:
            pid, image_id, img_path, caption, img_path_a, caption_a = self.dataset[index]
            img = read_image(img_path)
            ...
            caption_tokens_a = []
            for item in caption_a:
                caption_tokens_a.append(tokenize(item, tokenizer=self.tokenizer, text_length=self.text_length, truncate=self.truncate))
            if self.txt_aug:
                caption_tokens = self.txt_data_aug(caption_tokens.cpu().numpy())
                for i, _ in enumerate(caption_tokens_a):
                    caption_tokens_a[i] = self.txt_data_aug(caption_tokens_a[i].cpu().numpy())
            ...
            'images_a': img_a,
            'caption_ids_a': caption_tokens_a,
            'index':index,
            }
            if self.args.dataset_name=="AGTIReID":
                # print("DEBUG: AGTIReID dataset!")
                if "C4" in img_path or "C5" in img_path:
                    view=1
                else:
                    view=0
                ret.update({'views':view})

            return ret
        if len(self.dataset[0])==5:
            # print("DEBUG: ", self.dataset[index], flush=True)
            pid, image_id, img_path, caption, cam_id = self.dataset[index]
            # if cam_id is None:
            #     print("DEBUG: cam_id is None!", flush=True)
        else:
